...
// Enemy object inherit from GameObjects
Enemy.prototype = Object.create(GameObjects.prototype);

//set the constructor function of the Enemy Object to Enemy
...
Player.prototype.handleInput = function (userInput) {
    //check to see what arrowkey was clicked and moves the player object to the
...
function checkCollision(x, y) {
    ...
    allEnemies.forEach(ene => {
        if ((ene.x + pad >= coords.xBegin - pad && ene.x + pad <= coords.xEnd - pad) && (ene.y >= coords.yBegin && ene.y <= coords.yEnd)) {
            ...
            if (player.lives === 0) {
                alert(`GG! Your Score is ${player.score}`);
                player.score = 0;
                location.reload()
...
//disables the scroll bar being controlled by the arrowkeys
